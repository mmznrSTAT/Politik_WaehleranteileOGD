...
#dann flatten
wahlbetStadtZH <- flatten(json_wahlbet$wahlbeteiligung_stadtzh)
...
kanton <- data.frame(matrix(unlist(kanton)))
                     
kanton <- jsonlite::fromJSON(json_wahlbet$wahlbeteiligung_auf_kantonsebene)
wahlbetZH <- flatten(kanton)
